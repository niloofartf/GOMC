                 ...
                 gmx_bool bMDPformat)
{
    ...
    if (available(fp, ir, indent, title))
    {
        ...
        /* GJS - REDS - textdump code - 2/26/18 */

        PI("reds", ir->nstcomm);
        PI("preprod", ir->nstcomm);
        PR("a11", ir->a11);
        PR("b11", ir->b11);
        PR("c11", ir->c11);
...
void cmp_inputrec(FILE *fp, const t_inputrec *ir1, const t_inputrec *ir2, real ftol, real abstol)
{
    ...
    cmp_int(fp, "inputrec->reds", -1, ir1->reds, ir2->reds);
    cmp_int(fp, "inputrec->preprod", -1, ir1->preprod, ir2->preprod);
    cmp_real(fp, "inputrec->a11", -1, ir1->a11, ir2->a11, ftol, abstol);    
    cmp_real(fp, "inputrec->b11", -1, ir1->b11, ir2->b11, ftol, abstol);    
    cmp_real(fp, "inputrec->c11", -1, ir1->c11, ir2->c11, ftol, abstol);    
